...
export function MapWidget({ cases }: { cases: BackendCase[] }) {
  const mapContainerRef = useRef<HTMLDivElement | null>(null);
  const mapRef = useRef<maplibregl.Map | null>(null);
  const markersRef = useRef<maplibregl.Marker[]>([]);

  const hotspots = useMemo<ZoneHotspot[]>(() => {
    const grouped = new Map<string, { count: number; latSum: number; lngSum: number; reasons: Map<string, number> }>();

    for (const item of cases) {
      const zone = `${item.latitude.toFixed(2)}, ${item.longitude.toFixed(2)}`;
    ...
    hotspots.forEach((spot) => {
      ...
      const popup = new maplibregl.Popup({ offset: 18 }).setHTML(`
        <div style="min-width: 180px; font-family: sans-serif;">
          <div style="font-weight: 700; font-size: 14px; margin-bottom: 6px;">Zone ${spot.zone}</div>
    ...
    <div className="relative h-full min-h-[300px] overflow-hidden rounded-xl border border-slate-200 bg-slate-50/80 dark:border-indigo-500/10 dark:bg-[#050B14]/40">
      ...
      <div className="absolute left-4 top-4 rounded-2xl border border-slate-200 bg-white/85 px-4 py-3 shadow-md backdrop-blur dark:border-indigo-500/20 dark:bg-[#0A1222]/85">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
          <div>
            <p className="text-slate-500 dark:text-slate-400">Zones tracked</p>
      ...
      <div className="absolute bottom-4 left-4 rounded-2xl border border-slate-200 bg-white/85 px-4 py-3 shadow-md backdrop-blur dark:border-indigo-500/20 dark:bg-[#0A1222]/85">
        ...
        <p className="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100">
          {topZone ? `Zone ${topZone.zone}` : "No zone data"}
        </p>
        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
          {topZone ? `${topZone.count} cases | ${topZone.topViolation}` : "Markers will appear when case data is available."}
        </p>
      </div>
    </div>
  );
...
function getMarkerClasses(count: number) {
  if (count >= 18) return "flex h-11 w-11 items-center justify-center rounded-full border-2 border-white bg-red-500 text-sm font-bold text-white shadow-[0_10px_25px_rgba(239,68,68,0.45)]";
  if (count >= 10) return "flex h-10 w-10 items-center justify-center rounded-full border-2 border-white bg-amber-400 text-sm font-bold text-slate-900 shadow-[0_10px_25px_rgba(251,191,36,0.45)]";
